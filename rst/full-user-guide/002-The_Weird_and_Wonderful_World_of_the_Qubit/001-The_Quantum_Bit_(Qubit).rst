...
state. In the real device the errors are due to imperfect measurements and some residual heating of the qubit. 
...
To do this we need the concept of a *quantum gate*.  A single qubit quantum gate 
...
the original quantum state by the gate: :math:`|\psi'\rangle=U|\psi\rangle`. Here 
...
In the composer it is given by a green gate with a X in the middle, an example 
score file is below. Try running it. Did you find that, unlike in the
